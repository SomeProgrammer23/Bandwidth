...
public class OnHoverBorderII : MonoBehaviour
{
    ...
    void Update()
    {
        if(pickup == true)
        {
            if (Input.GetKeyDown(KeyCode.Mouse0))
            {
                ...
                heldItem = hand.transform.GetChild(0);
            }
        }
        if (held == true)
        {
        ...
        RaycastHit hit;
        if (Physics.Raycast(fpsCam.transform.position, Vector3.forward, out hit))
        {
            Vector3 hitPos = hit.point;
            heldItem.LookAt(hitPos);
        }
